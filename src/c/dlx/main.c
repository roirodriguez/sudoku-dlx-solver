...
int main(void) 
{
    struct Grid *grid = get_knut_paper_nosol_grid();
...
struct Grid *get_knut_paper_nosol_grid(void)
{
    ...
    // just remove row 0
    struct Node *row_ptr = grid->cols[2].down;
    struct Node *old;
    bool last_iter = false;
    while(true)
    {
        if (row_ptr == row_ptr->right)
            last_iter = true;
        
        row_ptr->down->up = row_ptr->up;
        row_ptr->up->down = row_ptr->down;
        row_ptr->left->right = row_ptr->right;
        row_ptr->right->left = row_ptr->left;
        row_ptr->column->size--;
        old = row_ptr;
